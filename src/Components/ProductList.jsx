...
setLoading(true)
   ...
   method:"GET"
     ...
     var priceValues = [productsList.filters.rangeFilters[0].filterValues[0].start,productsList.filters.rangeFilters[0].filterValues[0].end]
         ...
         if (filters[key].length > 0)
         {
            ...
            if (key === "category")
            {
             ...
             if (key === "price")
             {
                ...
                console.log(start)
                filteredProducts = filteredProducts.filter(product => product.price>start[0] && product.price<end[1])
               setProducts(filteredProducts)
                 setLoading(true)
            }
              if (key === "rating")
             {
                ...
                filteredProducts = filteredProducts.filter(product => Math.round(product.rating)>=filters["rating"][0])
               setProducts(filteredProducts)
                 setLoading(true)
            }
             if (key === "year")
             {
                ...
                filteredProducts = filteredProducts.filter(product => product.year===filters["year"][0])
               setProducts(filteredProducts)
                 setLoading(true)
            }
 ...
 return (
  <MainContainer>
   <FilterContainer>
          ...
          {showBrands &&
             <>
                  <Button style={btnStyle} variant="text" onClick={()=>dispatch({type:"ClearBrands"})}><DeleteOutlineIcon/></Button>
          <BrandContainer>
          ...
          {
             price &&
                <Price start={price[0]} end={price[1]} filters={filters} setFilters={setFilters}  />
         ...
         <Rating rating={rating} filters={filters} setFilters={setFilters}/>
          ...
          <Button style={btnStyle} onClick={()=>setFilters({...filters,rating:[]})}><DeleteOutlineIcon/></Button>
          <Line />
          <h4>Year</h4>
<Button style={btnStyle} onClick={()=>setFilters({...filters,year:[]})}><DeleteOutlineIcon/></Button>
          {
          yearValues.map((year)=>
        <Year  year={year} filters={filters} setFilters={setFilters}/>
)
          }
 <Button style={btnStyle} onClick={()=>setFilters({
      gender: [],
      category: [],
      brand: [],
      rating: [],
      price: [],
      year:[]
   })}>Clear All Filters<DeleteOutlineIcon/></Button>
   </FilterContainer>
   {loading ?
 <LoaderContainer>
